...
若没有 requirements.txt，可手动安装主要依赖：

bash
pip install fastapi[all] sqlalchemy aiomysql redis httpx python-dotenv passlib[bcrypt] uvicorn
3. 配置环境变量
...
所有需要登录的接口均需在请求头添加 Authorization: {token}。
