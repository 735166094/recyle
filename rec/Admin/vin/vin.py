...
class VinResponseParser:
    ...
    def get_model_images(self, model: Dict[str, Any]) -> List[str]:
        """获取车型图片URL列表"""
...
class VinConfigManager:
    ...
    def create_api_client(config=None):
        ...
        return VinAPIClient(config.username, config.password)

    @staticmethod
    def query_vin_with_config(vin_code, config=None, timeout=10):
        """使用配置查询VIN码"""
        try:
...
if __name__ == "__main__":
    ...
    # 创建API客户端
    api = VinAPIClient(USERNAME, PASSWORD)
